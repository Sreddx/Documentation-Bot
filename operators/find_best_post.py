...
class FindBestPost(BaseOperator):
    ...
    def find_best_post(self, params, ai_context):
        ...
        print(
            f"used_links = {used_links}\ntitle_link_dict = {title_link_dict}")
        title_link_dict = {title: url for title,
                           url in title_link_dict.items() if url not in used_links}
        ...
        ai_context.add_to_log(
            f"Analyzing {len(title_link_dict)} potential post(s).", self)
        ...
        if best_post_title not in title_link_dict and best_post_title.endswith('.'):
            ...
            best_post_title = best_post_title[:-1]

        best_post_link = title_link_dict.get(best_post_title, '')

        ai_context.add_to_log(
            f"The most relevant post to your query is titled: {best_post_title}. With Link: {best_post_link}", self)
