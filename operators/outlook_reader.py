...
class OutlookReader(BaseOperator):
    @staticmethod
    def declare_name():
    ...
    def run_step(self, step, ai_context: AiContext):
        ...
        email_data, uploaded_file_names = self.read_emails(
            email, password, mark_as_read, email_id, folder, ai_context)
        ai_context.set_output('email_data', email_data, self)
        ai_context.set_output('attached_file_names', uploaded_file_names, self)

    def read_emails(self, user: str, password: str, mark_as_read: bool, email_id: str, folder: str, ai_context):
        all_email_data = []
        all_uploaded_files = []
        try:
            ...
            if folder:
                ...
                # Check if desired folder is in the list of mailbox names
                folder_exists = any(
                    re.search(rf'"/" {re.escape(folder)}', mbox.decode()) for mbox in mailbox_list)
                if not folder_exists:
                    ai_context.add_to_log(f"Folder {folder} does not exist.")
                    return [], []

                # Now select the mailbox
                result, _ = mail.select(folder)
                if result != 'OK':
                    ai_context.add_to_log(f"Failed to select folder {folder}.")
                    return [], []
            else:
                mail.select("inbox")
